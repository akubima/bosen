...
class IndexView(ViewsComponentsABC):
    ...
    def render(self):
        # HERO SECTION
        ctk.CTkLabel(master=self.frame, text=os.getenv('APP_NAME'), text_color='#0069ff', font=ctk.CTkFont(size=44,weight='bold')).grid(row=0, column=0, columnspan=3)
        ctk.CTkLabel(master=self.frame, text='"'+os.getenv('APP_FULL_NAME')+'"', font=ctk.CTkFont(size=18)).grid(
        ...
        self.widget_keyword_category.grid(row=3, column=0, padx=5, pady=40)

        self.widget_keyword = ctk.CTkEntry(self.frame, placeholder_text='Search keyword...', height=40, width=300)
        self.widget_keyword.grid(row=3, column=1,padx=5,pady=40)

        ctk.CTkButton(self.frame, text='Search', height=40, width=100, command=lambda:controller.search.search(self.widget_keyword_category.get(), self.widget_keyword.get())).grid(row=3, column=2, padx=5, pady=40)
